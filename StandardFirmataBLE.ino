...
void setup() { // Initializes all inputs and outputs
...
void loop() { // Main loop (runs forever)
  ...
  // Gets the x, y and button state of the joystick
  xPosition = analogRead(xPin);
  yPosition = analogRead(yPin);
  buttonState = digitalRead(buttonPin);z
 ...
 if (yPosition >= 0 && yPosition <= 50){ // Foward
  ...
  // When it's dark, turn on the front green LEDs, else they stay off
  if (lightLevel <= 200){
